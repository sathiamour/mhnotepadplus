...
public class NotePadPlus extends Activity {
	...
	private void AddOneNoteInUserDefView(Intent intent)
	{
		...
		TextView Text = (TextView)factory.inflate(R.layout.userdefitem, null);
        Text.setBackgroundDrawable(new BitmapDrawable(HelperFunctions.GetAlpha1x1Bg(this, R.drawable.bg_userdef_note, 120, 120, ClrId)));
        Text.setOnTouchListener(new MyOnTouchListener(Text, RowId, Positions.size(), ScreenWidth, ScreenHeight));
        Text.setText(Title);
        Positions.add(new NotePos(RowId, ProjectConst.NegativeOne, ProjectConst.NegativeOne));
        MainLayout.addView(Text);
	}
	
	private void UpdateOneNoteInUserDefView(Intent intent)
	{
		Log.d("log","It is OK");
		// update one or delete one
		int Idx = intent.getIntExtra(OneNote.KEY_INDEX, ProjectConst.NegativeOne);
		Log.d("log","It is OK2");
		TextView Child = (TextView)MainLayout.getChildAt(MainLayout.getChildCount()-1);
		if( Idx == ProjectConst.NegativeOne )
		{
			// delete one
			Log.d("log","in delete one");
		...
		int ClrId = intent.getIntExtra(OneNote.KEY_DRAWABLE_ID, ProjectConst.Zero);
		Child.setText(Title);
		Child.setBackgroundDrawable(new BitmapDrawable(HelperFunctions.GetAlpha1x1Bg(this, R.drawable.bg_userdef_note, 120, 120, ClrId)));
		Child.invalidate();
		}
	}
               ...
               switch (eventaction) 
               {
                    case MotionEvent.ACTION_DOWN: // touch down so check if the
            	         ...
            	         if( ClickCount == 1 )  
            	         {
            	             ...
            	             if( secClick - firClick < 900 /*&& !IsMove*/ )
            	             {  
