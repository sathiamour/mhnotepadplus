    			...
    			if (NoteRowId != 0 ) {
		    		...
		    		if( Note.getCount() == 1 )
		    		{
		    		    if( !Pwd_Orignal.getText().toString().equals(Note.getString(Note.getColumnIndexOrThrow(OneNote.KEY_PWD))))
		    		    {
				            ...
				            NotesDb.close();
				            Intent EditNoteIntent = null;
				            int Type = Note.getInt(Note.getColumnIndexOrThrow(OneNote.KEY_NOTETYPE));
				            if( Type == OneNote.ListNote )
				        	    EditNoteIntent = new Intent(NotificationPwdDlgActivity.this, EditCheckListNoteActivity.class);
				            else if( Type == OneNote.TextNote )
				                EditNoteIntent = new Intent(NotificationPwdDlgActivity.this, EditNoteActivity.class);
